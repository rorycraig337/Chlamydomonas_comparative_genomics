...
Final check with Blobtools

```
minimap2 -ax map-pb -t48 v2_out/c_incerta_canu_v2.contigs.fasta canu_v2.reads.fa > c_incerta.canu_v2.reads.sam
samtools view -Sb c_incerta.canu_v2.reads.sam | samtools sort - > c_incerta.canu_v2.reads.bam
samtools index c_incerta.canu_v2.reads.bam

export BLASTDB=/scratch/ncbi/blastdbs
blastn -task megablast -query v2_out/c_incerta_canu_v2.contigs.fasta -db nt -outfmt '6 qseqid staxids bitscore std' -max_target_seqs 1 -max_hsps 1 -num_threads 32 -evalue 1e-25 -out c_incerta.canu_v2.vs.nt.mts1.hsp1.1e25.megablast.out

blobtools create -i v2_out/c_incerta_canu_v2.contigs.fasta -s c_incerta.canu_v2.reads.sam -t c_incerta.canu_v2.vs.nt.mts1.hsp1.1e25.megablast.out -o blobtools_canu_v2
blobtools view -i blobtools_canu_v2.blobDB.json
blobtools plot -i blobtools_canu_v2.blobDB.json
```

No contaminant contigs, extract reads used in final assembly version
...
samtools fasta -0 usable_reads.fa usable_reads.bam
samtools view -b -f4 c_incerta.canu_v2.reads.bam > unmapped_reads.bam
samtools fasta -0 unmapped_reads.fa unmapped_reads.bam
cat usable_reads.fa unmapped_reads.fa > canu_final.reads.fa
```
